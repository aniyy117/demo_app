...
encoder.prototype.encode = function ( str ) {
    if ( !str ) {
        throw Error( "can't encode a invalid string" );
    };
    var aUTF16CodeUnits = new Uint16Array( str.length );
    Array.prototype.forEach.call( aUTF16CodeUnits, function ( el, idx, arr ) { arr[idx] = str.charCodeAt( idx ); } );
    var sUTF16Base64 = base64EncArr( new Uint8Array( aUTF16CodeUnits.buffer ) );

    return sUTF16Base64;
};

encoder.prototype.decode = function ( str ) {
    if ( !str ) {
        throw Error( "can't decode a invalid string" );
    };
    var sDecodedString = String.fromCharCode.apply( null, new Uint16Array( base64DecToArr( str, 2 ).buffer ) );
